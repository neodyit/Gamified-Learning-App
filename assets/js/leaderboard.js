document.addEventListener('DOMContentLoaded', () => {
    ...
    const userData = JSON.parse(localStorage.getItem('user')) || { name: 'QuestMaster', xp: 3200, level: 20, grade: 7 };
    userXp.textContent = userData.xp;
    userLevel.textContent = userData.level;

    // Fetch leaderboard data from leaderboard.json
    fetch('assets/data/leaderboard.json')
        ...
        .then(data => {
            ...
            users.push({ name: userData.name, xp: userData.xp, grade: userData.grade, classSection: 'A' });
            renderLeaderboard();
            updateUserRank();
        })
        .catch(error => {
            console.error('Error loading leaderboard:', error);
            leaderboardTable.innerHTML = '<p class="muted text-center">Error loading leaderboard. Please try again.</p>';
        });
    ...
    function renderLeaderboard() {
        ...
        leaderboardTable.innerHTML = paginatedUsers.map((user, index) => `
